Shader "PixelArt/FoliageSway2D_Integrated"
{
    Properties
    {
        ...
        _IdleSwayAmount ("Idle Sway Amount", Range(0, 0.2)) = 0.02
        _IdleSwaySpeed ("Idle Sway Speed", Range(0, 5)) = 1.0
        _Smoothness ("Motion Smoothness", Range(0.5, 3)) = 1.5
        
        [Header(Bending)]
        _BendFromBase ("Bend From Base", Range(0, 0.5)) = 0.1
        _BendCurve ("Bend Curve Power", Range(1, 3)) = 2.0
        _MaxBendX ("Max Bend Horizontal", Range(0, 1)) = 0.3
        _MaxBendY ("Max Bend Vertical", Range(0, 1)) = 0.1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Attempt at organic sway using layered sine waves
            ...
            v2f vert(appdata v)
            {
                ...
                // Calculate height factor for bending
                // UV.y: 0 at bottom, 1 at top
                float heightFactor = saturate((v.texcoord.y - _BendFromBase) / (1.0 - _BendFromBase + 0.001));
                heightFactor = pow(heightFactor, _BendCurve);
                
                // Smooth the height factor for less harsh transitions
                ...
                // Smooth wind oscillation using layered waves
                ...
                // === IDLE SWAY ===
                // Organic layered movement even without wind
                ...
                float2 totalDisplacement = windBend + idleSway + interactionBend;
