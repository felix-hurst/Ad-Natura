...
The structure of the Module Guide (MG) for modules follows \cite{HoffmanStrooper1995},
with adaptations for Unity game engine components and 2D geometric operations.
The mathematical notation comes from Chapter 3 of \cite{HoffmanStrooper1995} and
...
\hline
\multirow{9}{*}{Software Decision} & Geometric Operations & \hyperref[M10]{M10}\\
...
& Slime Mold Simulation & \hyperref[M16]{M16}\\
...
\subsection{Slime Mold Algorithm Modules}
...
\paragraph{State Variables:}
\begin{itemize}
\item $agents : \text{seq of Agent}$ --- Buffer containing all agent positions, angles, and types
\item $trailMap : \text{Texture2D}$ --- 2D texture storing pheromone trail intensities
...
\item \mref{M16} $\rightarrow$ \mref{M3}, \mref{M17}
\item \mref{M17} $\rightarrow$ \mref{M18}
\item \mref{M18} $\rightarrow$ (none)
...
% Figure~\ref{FigUseHierarchy} shows the use hierarchy between modules organized by levels. Modules at higher levels use (depend on) modules at lower levels. An arrow from module A to module B indicates that A uses B.
...
% TODO: Replace with a diagram that actually connects modules to others with arrows
...
\begin{itemize}
\item \textbf{Display:} Standard monitor capable of rendering 2D graphics
\item \textbf{Input Devices:} Keyboard and mouse (primary), with potential future support for touchscreen
\item \textbf{Graphics:} GPU with support for Unity's 2D rendering pipeline
\end{itemize}
...
\item \textbf{Milestones:} Major deliverables aligned with course deadlines (Rev0, Rev1 demonstrations)
\end{itemize}
