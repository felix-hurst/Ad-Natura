...
This document uses mathematical notation to specify module interfaces. The following table summarizes the primitive data types used throughout this document. For a complete notation reference including geometric functions and operators, see Section~\ref{SecNotation}.
...
\hline
positive real & $\mathbb{R}^+$ & Any positive real number in $(0, \infty)$ \\
\hline
boolean & $\mathbb{B}$ & A truth value: true or false \\
\hline
sequence & seq of T & An ordered list of elements of type T \\
\hline
2D vector & Vector2 & A tuple $(x: \mathbb{R}, y: \mathbb{R})$ representing a point or direction \\
\hline
\end{tabular}
...
\item[\refstepcounter{acnum} \actheacnum \label{AC7}:] Algorithm used for irregular edge generation (currently supports sine wave interpolation for smooth materials, random offset for jagged materials, and hybrid approaches for medium materials).
...
\item[\refstepcounter{ucnum} \uctheucnum \label{UC5}:] The fundamental geometric representation of objects as polygons with vertices (distinct from AC4 which addresses material behavioural properties like softness; UC5 concerns the underlying data structure for shape representation)
...
\midrule
& Slime Mold Simulation module\\
\midrule
& Slime Mold Manager module\\
\midrule
& Attraction Source module\\
...
\begin{description}
\item[DD1: Dual Representation] All cut objects have a pixelated mesh texture using Bresenham algorithm applied to them in contrast with the smooth collision mesh even if the cut was irregular and/or diagonal in order to ensure adherence to the visual style (R6). \textit{Related modules: \mref{M12}, \mref{M13}}

\item[DD2: Material Profile System] In order to meet the requirement for different cutting behaviour on different materials (R5) a JSON-based configuration system maps material names/tags to extendable cutting parameters (softness and strength). \textit{Related modules: \mref{M9}, \mref{M15}}

\item[DD3: Modular Cutting Pipeline] Cutting object flow can be broken into the following key phases of visualization, partitioning, mesh generation, and debris spawning thereby allowing each phase to be optimized independently and meeting the requirement for real-time interactive cutting (R3) in an extendable and maintainable way. \textit{Related modules: \mref{M6}, \mref{M7}, \mref{M11}, \mref{M12}, \mref{M8}}

\item[DD4: Entry-Exit Point Model] In order for players to have a clear understanding of what the tool destroys (R2), cuts are defined by exactly two intersection points for each object (entry and exit) which simplifies the logic and provides clear visualization and working feedback to the player. \textit{Related modules: \mref{M6}, \mref{M7}}

\item[DD5: Area-Proportional Mass] For realistic physics behaviour after cutting (R4), object mass is updated proportionally to the change in area, maintaining consistent density. \textit{Related modules: \mref{M10}, \mref{M14}}
\end{description}
...
\item The Raycast module (M6) is initialized before player input is processed
\end{itemize}
...
\item Cut profiles are loaded and available from M9 before cuts are executed
...
\paragraph{Services}
Discovers attraction sources in scene hierarchy, generates 2D attraction maps with radial falloff from each source, coordinates with M16 to provide attraction data for agent steering, supports real-time source addition/removal.
...
\item The slime mold simulation module (M16) is initialized before maps are provided
\end{itemize}
...
\item The slime mold manager (M17) will periodically query all active sources
\end{itemize}
...
The user interface for the Destructible Environment System is designed for intuitive interaction with the 2D game world.
...
The structure of the Module Interface Specification (MIS) for modules follows \cite{HoffmanStrooper1995}, with adaptations for Unity game engine components and 2D geometric operations. The mathematical notation comes from Chapter 3 of \cite{HoffmanStrooper1995} and standard computational geometry references. For instance, the symbol := is used for assignment statements and conditional rules follow the form $(c_1 \Rightarrow r_1 | c_2 \Rightarrow r_2 | ... | c_n \Rightarrow r_n)$.
